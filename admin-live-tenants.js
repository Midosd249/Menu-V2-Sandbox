...
(function(){
  ...
  async function loadAuthorizedTenants(){
    ...
    if(typeof authorizedTenantSlugs!=='undefined'){
      authorizedTenantSlugs=liveOptions.map(x=>x.slug);
    }
    const el=select();
    if(!el)return false;
    ...
    el.disabled=liveOptions.length===1;
  ...
  function bindSelector(){
    ...
    el.onchange=async function(e){
      ...
      selectedBranchSlug='main';
      localStorage.setItem('adminBranch',selectedBranchSlug);
      window.editId=null;
      branchList=[{id:'demo-'+selectedBranchSlug,slug:selectedBranchSlug,name:'الفرع الرئيسي',address:'الرياض',maps:'https://maps.google.com/?q=Riyadh',is_active:true}];
