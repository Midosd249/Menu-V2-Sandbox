...
(function(){
  ...
  async function loadTenantBySlug(slug){
    ...
    try{
      ...
      db[tenant]={
        brand:{
          ar:t.name||'', en:t.name||'',
          taglineAr:t.tagline||'', taglineEn:t.tagline||'',
          maps:'', whatsapp:t.whatsapp||'',
          waTemplate:t.whatsapp_message_template||'',
          instagram:t.instagram_url||'',
          primaryColor:t.primary_color||'#15120f',
          secondaryColor:t.secondary_color||'#a26a42',
          logoUrl:t.logo_url||'', coverUrl:t.cover_url||''
        },
        items:(products.data||[]).map(p=>({
          id:p.id, ar:p.name_ar||'', en:p.name_en||'',
          descAr:p.description_ar||'', descEn:p.description_en||'',
          price:Number(p.price||0), cat:p.category_id,
          available:p.is_available!==false, featured:!!p.is_featured,
          image_url:p.image_url||'', currency:p.currency||'SAR',
  ...
  /* Replace the previous single-tenant guard with membership-scoped switching. */
  ...
  /* Branches are tenant-scoped navigation; products/categories are tenant-scoped in the current schema. */
  ...
  /* Ensure health checks the selected branch rather than the first branch. */
  const originalComputeHealth=window.computeHealth;
  if(typeof originalComputeHealth==='function'){
    window.computeHealth=function(){
      ...
      if(isLive && branchList.length){
        ...
        for(const c of checks){
          if(c.label==='عنوان الفرع ناقص') c.ok=address;
          if(c.label==='عنوان الفرع موجود') c.ok=address;
          if(c.label==='رابط خريطة الفرع ناقص') c.ok=maps;
          if(c.label==='خريطة الفرع موجودة') c.ok=maps;
        }
      }
      return result;
    };
